...
const TemplatesSection = () => {
  ...
  const categoryOrder = [
    "estatico", "carrossel", "reels", "branding", "marca", "ebook", "outros",
  ];
    ...
    if (draggedService.category === targetService.category) {
      ...
      await Promise.all(
        reordered.map((service, index) =>
          updateOrderMutation.mutateAsync({ id: service.id, display_order: index })
        )
      );
    } else {
      const { error } = await supabase
  ...
  return (
    <div className="space-y-6">
      <style>{`
        .drag-item { will-change: transform, opacity; }
        .drag-item.dragging { opacity: 0.4; transform: scale(0.95) rotate(2deg); }
        .drag-item.drag-over {
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div className="flex items-center gap-4">
          <div>
            <h2 className="text-2xl font-bold">Templates de Serviços</h2>
            <p className="text-sm text-muted-foreground mt-1">
              Biblioteca reutilizável — copie templates para clientes na aba "Clientes".
            </p>
          </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                      {categoryServices.map((service) => (
                        <div
                          ...
                          } hover:scale-[1.02] active:scale-[0.98] drag-item`}
                        >
                          <ServiceCard
                            service={service}
                            onEdit={handleEdit}
      ...
      <Dialog open={isFormOpen} onOpenChange={handleFormClose}>
        <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>{editingService ? "Editar template" : "Novo template"}</DialogTitle>
          </DialogHeader>
