...
const ServiceSteps = () => {
  ...
  return <section ref={sectionRef} className="relative py-16 md:py-20 bg-background" style={{
    minHeight: `${steps.length * 60}vh`
      ...
      <div className="sticky top-0 h-screen flex items-center justify-center py-8">
        ...
        <div className={`hidden lg:block absolute left-8 top-1/2 -translate-y-1/2 z-10 transition-opacity duration-300 ${isInView ? "opacity-100" : "opacity-0 pointer-events-none"}`}>
        ...
        <div className="container mx-auto px-4 md:px-6">
          <h2 className="text-3xl md:text-4xl lg:text-5xl font-bold text-center mb-12 md:mb-20 text-foreground">Como Funcionam Meus Serviços</h2>

          <div className="max-w-3xl mx-auto relative">
            {steps.map((step, index) => <div key={index} className={`transition-all duration-700 ${currentStep === index ? "opacity-100 scale-100" : "opacity-0 scale-95 absolute inset-0 pointer-events-none"}`}>
                <div className="bg-card border border-border rounded-2xl p-6 md:p-8 lg:p-12 shadow-lg mx-4 md:mx-0">
                  ...
                  <div className="flex justify-center gap-1.5 md:gap-2 mt-6 md:mt-8">
                    {steps.map((_, idx) => <div key={idx} className={`h-2 rounded-full transition-all duration-300 ${idx === currentStep ? "w-8 bg-primary" : "w-2 bg-border"}`} />)}
                  </div>
                </div>
              </div>)}
          </div>
