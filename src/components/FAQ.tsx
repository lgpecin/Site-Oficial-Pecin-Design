...
const FAQ = () => {
  ...
  const services = [{
    ...
    icon: BookOpen,
    title: "Criação de Ebooks"
  }, {
    icon: TrendingUp,
    title: "Planejamento Estratégico"
  }, {
    icon: Package,
    title: "Design de Produtos e Embalagens"
  }];
  const faqs = [{
    question: "Quais serviços de design podemos desenvolver?",
    answer: <div>
          <p className="mb-6">Trabalho com bastante coisa mesmo! A gente sempre pode conversar sobre projetos mais diferentes, mas geralmente, o que costumo desenvolver são os itens abaixo:</p>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {services.map((service, index) => <div key={index} className="flex flex-col items-center gap-3 p-4 rounded-lg bg-muted/50 hover:bg-muted transition-colors">
  ...
  return <section id="faq" className="py-12 sm:py-16 bg-muted/30">
      <div className="container mx-auto px-6">
        ...
        <div className="max-w-3xl mx-auto">
          <Accordion type="single" collapsible className="space-y-4">
            {faqs.map((faq, index) => <AnimatedSection key={index}>
